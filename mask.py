...
def MAR_mask(X, p, p_obs, selfmasked=False):
    """
    ...
    n, d = X.shape  # n代表样本数，d代表feature数
    ...
    d_obs = max(
        int(p_obs * d), 1
    )  ## number of variables that will have no missing values (at least one variable)  # 观测到的feature数
    d_na = (
        d - d_obs
    )  ## number of variables that will have missing values   # 未观测到的feature数

    ### Sample variables that will all be observed, and those with missing values:
    idxs_obs = np.random.choice(
        d, d_obs, replace=False
    )  # 随机采样一个feature子集，代表观测到的feature
    idxs_nas = np.array(
        [i for i in range(d) if i not in idxs_obs]
    )  # 剩余的就是缺失的feature集合
    ...
    coeffs = pick_coeffs(X, idxs_obs, idxs_nas, selfmasked)
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X[:, idxs_obs], coeffs, p, selfmasked)

    ps = torch.sigmoid(X[:, idxs_obs].mm(coeffs) + intercepts)  # 返回0-1的阈值

    ber = torch.rand(n, d_na)
    mask[:, idxs_nas] = (
        ber < ps
    )  # 小于阈值的就代表缺失，为True，大于阈值的代表非缺失，为False
...
def fit_intercepts(X, coeffs, p, self_mask=False):
    if self_mask:
        ...
        for j in range(d):
            ...
            intercepts[j] = optimize.root(f, 0).x[0]
    else:
        ...
        for j in range(d_na):
            ...
            # intercepts[j] = optimize.bisect(f, -50, 50) # scipy.optimize.bisect用于求解标量函数的单个根，区间在[-50,50]上
            intercepts[j] = optimize.root(f, 0).x[
                0
            ]  # METR-LA的MNAR和MAR情况下只能用scipy.optimize.root计算
    return intercepts
